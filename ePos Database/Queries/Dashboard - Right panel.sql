-- -- TODAYS METRICS
    ...
    -- @param : Rest_ID ; ID of the restaurant
		SELECT IFNULL(COUNT(ID),0) FROM ORDERS WHERE DATE(ENDED) = DATE(SYSDATE()) AND REST_ID = 1 AND ACTIVE =0;

	-- Total Sales for today
	-- @param : Rest_ID ; ID of the restaurant
		SELECT IFNULL(SUM(TOTAL),0) FROM ORDERS WHERE DATE(ENDED) = DATE(SYSDATE()) AND REST_ID =1 AND ACTIVE =0;
    ...
    -- Percentage equation is simplied (A-B)/B*100 = (A/B - 1)*100
		SELECT
			IFNULL((((
				(SELECT IFNULL(SUM(TOTAL),0) FROM ORDERS WHERE DATE(ENDED) = DATE(SYSDATE()) AND REST_ID =1 AND ACTIVE =0)
				/ 
				(SELECT IFNULL(SUM(TOTAL),0) FROM ORDERS WHERE DATE(ENDED) = DATE(SUBDATE(SYSDATE(),1)) AND REST_ID =1 AND ACTIVE =0)
			) -1 ) *100 ) ,0)
			AS PERCENTAGE
		FROM DUAL 
...
-- -- TO DATE's METRICS
	...
	-- @param : Rest_ID ; ID of the restaurant
		SELECT IFNULL(COUNT(ID),0) FROM ORDERS WHERE YEAR(ENDED) = YEAR(SYSDATE()) AND REST_ID =1 AND ACTIVE =0;

	-- Total Sales this year
	-- @param : Rest_ID ; ID of the restaurant
		SELECT IFNULL(SUM(TOTAL),0) FROM ORDERS WHERE YEAR(ENDED) = YEAR(SYSDATE()) AND REST_ID =1 AND ACTIVE =0;
...
-- -- CUSTOMER METRICS
    ...
    -- @param : Rest_ID ; ID of the restaurant
		SELECT IFNULL(COUNT(C.ID),0) FROM 
        CUSTOMERS C
